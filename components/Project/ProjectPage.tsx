...
export const ProjectPage = ({ data }) => {
  ...
  const img = urlFor(image.src).auto('format').width(isMobile ? 600 : 1800).dpr(2).quality(90).url();
        ...
        <Layout>
          ...
          >
            ...
            {images && images.map((image, index) => {
              const img = urlFor(image).auto('format').width(isMobile ? 600 : 1800).dpr(2).quality(90).url();
